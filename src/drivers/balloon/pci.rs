...
impl VirtioBalloonDriver {
	...
	pub fn from_pci_device(
		...
		let mut driver = VirtioBalloonDriver::new(caps, device)
			.inspect_err(|_| {
				error!("Initializing new traditional memory balloon driver failed. Aborting!");
			})
		...
		info!(
			"VIRTIO traditional memory balloon device with id {:x}, has been initialized by driver!",
			driver.get_dev_id()
		);
	...
	pub fn handle_interrupt(&mut self) {
		...
		if status.contains(IsrStatus::DEVICE_CONFIGURATION_INTERRUPT) {
			debug!("Received config interrupt, new config: {:?}", self.dev_cfg);
		}

		if status.contains(IsrStatus::QUEUE_INTERRUPT) {
			debug!("Received queue interrupt");
		}
