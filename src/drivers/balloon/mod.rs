	...
	pub fn allocate_chunks(
		...
		loop {
			let Some(current_pow_2_nz) = NonZeroU32::new(current_pow_2) else {
				if num_remaining > 0 {
					warn!(
						"<balloon> Failed to allocate new pages to fill the balloon with, continuing with as many as possible"
					);
				}
				break;
			};
			...
			match self.allocate_chunk(talc, current_pow_2_nz) {
				...
				Err(()) => {
					...
					trace!(
						"<balloon> Failed to allocate new chunk of {old_pow_2} pages to fill the balloon with, reducing chunk size to {current_pow_2}"
					);
